...
def update_experiment_numbers(bin_files_dir, excel_file_path):
    ...
    bin_files = [f for f in os.listdir(bin_files_dir) if f.endswith('.bin')]

    if not bin_files:
        print("Klasörde .bin dosyası bulunamadı.")
        return

    for bin_file in bin_files:
        try:
            # Dosya adından kişi ismini çıkar
            match = re.match(r"(.*?)_deney\d+_tekrar\d+_timestamp\d+\.bin", bin_file)
            if not match:
